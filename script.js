...
const description=elements("p","Do mathametical expression using this calculator","id","description");
...
div.append(input,clear,back,dot,multiply,seven,eight,nine,divide,four,five,six,minus,one,two,three,plus,zero,doublezero,equal);
main.append(div);
...
document.addEventListener("keydown", (entnum) => {
    ...
    if (allowedKeys.test(entnum.key)) {  //testing if the values are in allowed keys
        input.value += entnum.key;
    } else if (entnum.key === 'Enter') { // Check for Enter key to evaluate
...
one.addEventListener('click', function() { buttonClick('1'); });
two.addEventListener('click', function() { buttonClick('2'); });
three.addEventListener('click', function() { buttonClick('3'); });
four.addEventListener('click', function() { buttonClick('4'); });
five.addEventListener('click', function() { buttonClick('5'); });
six.addEventListener('click', function() { buttonClick('6'); });
seven.addEventListener('click', function() { buttonClick('7'); });
eight.addEventListener('click', function() { buttonClick('8'); });
nine.addEventListener('click', function() { buttonClick('9'); });
zero.addEventListener('click', function() { buttonClick('0'); });
doublezero.addEventListener('click', function() { buttonClick('00'); });
plus.addEventListener('click', function() { buttonClick('+'); });
minus.addEventListener('click', function() { buttonClick('-'); });
divide.addEventListener('click', function() { buttonClick('/'); });
dot.addEventListener('click', function() { buttonClick('.'); });
multiply.addEventListener('click', function() { buttonClick('*'); });
